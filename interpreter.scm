...
(define interpret_function
  (lambda (body state break continue throw type_name instance_name function_type_name)
    (call/cc
     (lambda (return)
       (cond
         ((null? body) state)
         (else (interpret_function (next_statements body) (M_state (current_statement body) state break continue throw return type_name instance_name) break continue throw type_name instance_name function_type_name)))))))
...
(define M_state
  (lambda (statement state break continue throw return class_type instance_name)
    (cond
      ((null? statement) state)
      ((number? statement) statement)
      ((state_declared? (operator statement) state) (state_lookup_variable (operator statement) state))
      ((eq? (operator statement) 'var) (M_state_declare (rhs statement) (lhs statement) state break continue throw return class_type instance_name))
      ((eq? (operator statement) '=) (M_state_assign (rhs statement) (lhs statement) state break continue throw return class_type instance_name))   
      ((eq? (operator statement) 'if) (if (null? (else_exists statement))
                                          (M_state_if_else (conditional statement) (if* statement) empty state break continue throw return class_type instance_name)
                                          (M_state_if_else (conditional statement) (if* statement) (else statement) state break continue throw return class_type instance_name)))
      ((eq? (operator statement) 'return) (M_state_return (rhs statement) state break continue throw return class_type instance_name))
      ((eq? (operator statement) 'while) (M_state_while (conditional statement) (loop statement) state break continue throw return class_type instance_name))
      ((eq? (operator statement) 'begin) (M_state_begin (block statement) state break continue throw return class_type instance_name))
      ((eq? (operator statement) 'break) (M_state_break state break class_type instance_name))
      ((eq? (operator statement) 'continue) (M_state_continue state continue class_type instance_name))
      ((eq? (operator statement) 'throw) (M_state_throw (cadr statement) state break continue throw return class_type instance_name))
      ((eq? (operator statement) 'try) (M_state_try (cadr statement) (caddr statement) (cadddr statement) state break continue throw return class_type instance_name))
      ((or (eq? (operator statement) 'function) (eq? (operator statement) 'static-function)) (M_state_function_add (name statement) (parameters statement) (body statement) state break continue throw return class_type instance_name))
      ((eq? (operator statement) 'funcall) (M_state_function (name statement) (arguments statement) state break continue throw return class_type instance_name))
      ((eq? (operator statement) 'class) (M_state_class_add (name statement) (cddr statement) state break continue throw return class_type instance_name))
      (else (error "Invalid operator:" (operator statement))))))

(define M_state_class_add
  (lambda (name code state break continue throw return class_type instance_name)
...
(define M_state_declare
  (lambda (variable expression state break continue throw return class_type instance_name)
    (if (null? expression)
        (state_bind variable '? state)
        (state_bind variable (M_value (car expression) state break continue throw return class_type instance_name) state))))
    ...
    (if (and (list? (car expression)) (eq? '= (caar expression)))
        (state_update
         ...
         (state_lookup_variable
          ...
          (M_state_assign (next_variable expression) (next_expression expression) state break continue throw return class_type instance_name))
         (M_state_assign (next_variable expression) (next_expression expression) state break continue throw return class_type instance_name))
        (if (eq? (car variable) 'dot)
            (state_update_instance (caddr variable) instance_name (M_value (car expression) state break continue throw return class_type instance_name) state)
            (state_update variable (M_value (car expression) state  break continue throw return class_type instance_name) state)))))
...
(define M_state_if_else
  (lambda (condition if else state break continue throw return class_type instance_name)
    (cond
      ((M_boolean condition state break continue throw return class_type instance_name) (M_state if state break continue throw return class_type instance_name))
      ((not (null? else)) (M_state else state break continue throw return class_type instance_name))
      (else state))))
...
(define M_state_while
  (lambda (condition statement state break continue throw return class_type instance_name)
    (call/cc
     (lambda (break)
       (M_state_while_loop condition statement state break continue throw return class_type instance_name)))))
...
(define M_state_while_loop
  (lambda (condition statement state break continue throw return class_type instance_name)
    (if (M_boolean condition state break continue throw return class_type instance_name)
        (M_state_while_loop condition statement (call/cc
                                            (lambda (continue)
                                              (M_state statement state break continue throw return class_type instance_name)))
                            break continue throw return class_type instance_name) state)))

; Begins a block of statements to be evaulated
(define M_state_begin
  (lambda (block state break continue throw return class_type)
    (if (null? block)
        state
        (remove_layer (M_state_begin_block block (cons new_layer state) break continue throw return class_type instance_name)))))

; Helper function for begin
(define M_state_begin_block
  (lambda (block state break continue throw return class_type instance_name)
    (if (null? block)
        state 
        (M_state_begin_block (next_statements block) (M_state (current_statement block) state break continue throw return class_type instance_name) break continue throw return class_type instance_name))))
...
(define M_state_try
  (lambda (try catch finally state break continue throw return class_type instance_name)
       (letrec ((attempt
                 (lambda (try state break continue return)
                   (call/cc
                    (lambda (throw)
                      (M_state_begin_block try (cons new_layer state) break continue throw return class_type instance_name)))))
                (e (attempt try state break continue return class_type instance_name)))
         (if (atom? e)
             (cond
               ((and (null? catch) (null? finally)) state)
               ((null? catch) (M_state_begin_block (cadr finally) state break continue throw return class_type instance_name))
               (else (remove_layer (M_state_begin_block (cadr finally)
                                          (M_state_begin_block (caddr catch)
                                                               (M_state_declare (caadr catch) (list e) (cons new_layer state) break continue throw return class_type instance_name)
                                                               break continue throw return class_type)
                                          break continue throw return class_type instance_name))))
             (if (null? finally)
                 (remove_layer e)
                 (remove_layer (M_state_begin_block (cadr finally) e break continue throw return class_type instance_name)))))))

; Break continuation
(define M_state_break
  (lambda (state break class_type instance_name)
    (break (remove_layer state))))

; Continue continuation
(define M_state_continue
  (lambda (state continue class_type instance_name)
    (continue (remove_layer state))))

; Throw continuation
(define M_state_throw
  (lambda (expression state break continue throw return class_type instance_name)
    (throw (M_value expression state break continue throw return class_type instance_name))))

; Return continuation
(define M_state_return
  (lambda (expression state break continue throw return class_type instance_name)
    (if (and (not (atom? expression)) (is_boolean? (operator expression)))
        (return (convert_value (M_boolean expression state break continue throw return class_type instance_name)))
        (return (convert_value (M_value expression state break continue throw return class_type instance_name))))))
...
(define M_state_function_add
  (lambda (name parameters statement state break continue throw return class_type instance_name)
...
(define M_state_function
  (lambda (name arguments state break continue throw return class_type instance_name)
    (begin (M_value_function name arguments state break continue throw return class_type instance_name) state)))
    ...
    (cond
      ((atom? expression)
       (cond
         ...
         ((is_boolean? expression) (M_boolean expression state break continue throw return class_type instance_name))
      ...
      ((is_boolean? (operator expression)) (M_boolean expression state break continue throw return class_type instance_name))
      ((not (is_operator? (operator expression))) (error "Variable not declared:" (operator expression)))
      ((eq? (operator expression) 'funcall) (M_value_function (operand_1 expression) (cddr expression) state break continue throw return class_type instance_name))
      ((eq? (operator expression) 'new) (M_value_class (cadr expression) state break continue throw return class_type instance_name))
      ((eq? (operator expression) 'dot) (if (eq? (operand_1 expression) 'this)
                                            (M_value_class_this (operand_2 expression) state break continue throw return class_type instance_name)
                                            (M_value_class_field (operand_1 expression) (operand_2 expression) state break continue throw return class_type instance_name)))
      ((eq? (operator expression) '+) (round (+ (M_value (operand_1 expression) state break continue throw return class_type instance_name)
                                                (M_value (operand_2 expression) state break continue throw return class_type instance_name))))
      ((eq? (operator expression) '-) (if (not (null? (cddr expression)))
                                          (round (- (M_value (operand_1 expression) state break continue throw return class_type instance_name)
                                                    (M_value (operand_2 expression) state break continue throw return class_type instance_name)))
                                          (round (* (M_value (operand_1 expression) state break continue throw return class_type instance_name)
                                                    -1))))
      ((eq? (operator expression) '*) (round (* (M_value (operand_1 expression) state break continue throw return class_type instance_name)
      ...
      ((eq? (operator expression) '/) (round (/ (M_value (operand_1 expression) state break continue throw return class_type instance_name)
                                                (M_value (operand_2 expression) state break continue throw return class_type instance_name))))
      ((eq? (operator expression) '%) (round (modulo (M_value (operand_1 expression) state break continue throw return class_type instance_name)
                                                     (M_value (operand_2 expression) state break continue throw return class_type instance_name))))
...
(define M_value_function
  (lambda (name arguments state break continue throw return class_type instance_name)
    (if (null? instance_name)
        (interpret_function (cadr (state_lookup_variable (caddr name) (state_lookup_variable (cadr (class_dot_type (cadr name) state)) state)))
                            ...
                            (cadr (class_dot_type (cadr name) state))) ; NOT RIGHT
        (interpret_function (cadr (state_lookup_variable (caddr name) state))
                            (function_environment (caddr name) arguments
                                                  state
                                                  break continue throw return)
                            break continue throw
                            class_type
                            instance_name
                            class_type)))) ; NOT RIGHT

(define M_value_class
  (lambda (type state break continue throw return class_type instance_name)
...
(define M_value_class_field
  (lambda (instance field state break continue throw return class_type instance_name)
    (state_lookup_instance instance field state)))

(define M_value_class_this
  (lambda (field state break continue throw return class_type instance_name)
...
(define M_boolean
  (lambda (condition state break continue throw return class_type instance_name)
    (cond
      ((eq? condition 'true) #t)
      ((eq? condition 'false) #f) 
      ((eq? (operator condition) '<=) (<= (M_value (operand_1 condition) state break continue throw return class_type instance_name)
                                          (M_value (operand_2 condition) state break continue throw return class_type instance_name)))
      ((eq? (operator condition) '>=) (>= (M_value (operand_1 condition) state break continue throw return class_type instance_name)
                                          (M_value (operand_2 condition) state break continue throw return class_type instance_name)))
      ((eq? (operator condition) '<) (< (M_value (operand_1 condition) state break continue throw return class_type instance_name)
                                        (M_value (operand_2 condition) state break continue throw return class_type instance_name)))
      ((eq? (operator condition) '>) (> (M_value (operand_1 condition) state break continue throw return class_type instance_name)
                                        (M_value (operand_2 condition) state break continue throw return class_type instance_name)))
      ((eq? (operator condition) '==) (eq? (M_value (operand_1 condition) state break continue throw return class_type instance_name)
                                           (M_value (operand_2 condition) state break continue throw return class_type instance_name)))
      ((eq? (operator condition) '!=) (not (eq? (M_value (operand_1 condition) state break continue throw return class_type instance_name)
                                                (M_value (operand_2 condition) state break continue throw return class_type instance_name))))
      ((eq? (operator condition) '||) (if (or (M_value (operand_1 condition) state break continue throw return class_type instance_name)
                                              (M_value (operand_2 condition) state break continue throw return class_type instance_name))
                                     #t
                                     #f))
      ((eq? (operator condition) '&&) (if (and (M_value (operand_1 condition) state break continue throw return class_type instance_name)
                                               (M_value (operand_2 condition) state break continue throw return class_type instance_name))
                                     #t
                                     #f))
      ((eq? (operator condition) '!) (not (M_value (operand_1 condition) state break continue throw return class_type instance_name)))
